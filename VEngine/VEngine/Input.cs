...
{
    private static List<Key> _keysDown = new();
    private static List<Key> _keysDownLastFrame = new();
    
    private static List<MouseButton> _mouseButtonsDown = new();
    private static List<MouseButton> _mouseButtonsDownLastFrame = new();
    
    private static Vector2 _mousePosition;
    private static Vector2 _mousePositionLastFrame;
    
    public static Vector2 MousePosition => _mousePosition;
    public static Vector2 MouseDelta => _mousePosition - _mousePositionLastFrame;
    
    public static MouseMode MouseMode { get; set; } = MouseMode.Normal;
    private static MouseMode _mouseModeLastFrame = MouseMode.Normal;

    #region Engine

    public delegate void MouseModeCallback(MouseMode mode);
    public static MouseModeCallback? OnMouseModeChanged;
    
    public static void Update()
    {
        _keysDownLastFrame = new List<Key>(_keysDown);
        _mouseButtonsDownLastFrame = new List<MouseButton>(_mouseButtonsDown);
        _mousePositionLastFrame = _mousePosition;
        
        if (MouseMode != _mouseModeLastFrame)
        {
            OnMouseModeChanged?.Invoke(MouseMode);
        }
        _mouseModeLastFrame = MouseMode;
    }
    ...
    // What follows is the actual input API

    public static bool GetKey(Key p0)
    {
        return _keysDown.Contains(p0);
    }

    public static bool GetMouseButton(MouseButton right)
    {
        return _mouseButtonsDown.Contains(right);
    }
}
